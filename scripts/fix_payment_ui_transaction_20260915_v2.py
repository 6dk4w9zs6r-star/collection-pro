...
start = text.find('async function mfApplyPayment')
end = text.find('async function mfUpdatePaymentStatus', start)
if start < 0 or end < 0:
    raise SystemExit('payment function markers not found')

replacement = '''async function mfApplyPayment(c,amount,date,type,status="successful",note="",receipt=null,source="manual"){
...
if '## 2026-09-15 — Full Audit: protected profile team membership' not in log:
    log_path.write_text(log + entries, encoding='utf-8')
