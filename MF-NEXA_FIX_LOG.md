...
- Added password-encrypted operational archives (AES-256-GCM, PBKDF2-SHA256, random salt/IV), authenticated decryption and project/table/count validation. No password is persisted or sent to the server. Local financial Restore is removed; validation is read-only and explicit canonical reload reads Supabase. Full recovery remains an uncompleted gate with unavailable direct backup-management access.
- Added versioned usage consents with server policy text/time, immutable ownership and an idempotent RPC. Local-only consent no longer bypasses the database check. A stale read cannot overwrite a just-saved consent.
- Added server-stamped explicit login/logout Audit events using the authenticated session id and a unique event key. Tokens/passwords are never recorded; failed logout audit does not prevent the original sign-out path.
- Migrations: `operational_archive_and_durable_consent`, `durable_session_audit`. Rollback-only tests passed for export scope/count/audit, immutable consent/active accounts and session attribution/uniqueness/privacy. No operational test records retained. Targeted local suite and browser evidence are in the test scripts; real authenticated production E2E remains open.

# 2026-09-16 — Messaging, notifications and call authorization, release r5

- Chat inserts use a stable client message id and an invoker RPC; retries return the same row and reject changed content under the same id. The server validates sender/name/time, canonical room participants, registered branch/team scope and sender-owned attachment paths. A private boolean contact lookup checks active recipients without broadening profile visibility.
- Chat reloads are paginated; failed reads are reported. Local send completion and Realtime inserts merge by id. Draft text and selected attachment survive same-room rerender. Private attachments receive fresh signed URLs. Invented fallback team rooms and non-account private targets are removed.
- Personal notification settings save only after a returned database row, and reload from Supabase. Read actions confirm the saved recipient-owned notification and use its server client/link fields. Load failures no longer display a misleading empty center. Disabled users cannot read or update notifications/preferences/calls/chat.
- New calls start ringing; only the callee accepts/rejects, the caller may cancel, and accepted participants may end. Ended calls cannot reopen. The UI checks the accepted invitation before media setup, requests private Realtime signaling, and handles authorization/timeout errors. Private topic RLS only authorizes active participants of an accepted call; Founder archive access does not grant participation. Actual authenticated WebSocket/audio/video/device E2E and global public-channel settings remain unverified.
- Morning content is visible only for its publication/scheduled day in Asia/Amman. Previous-day content remains stored but is not presented as today's morning content.
- Migrations in applied order: `chat_notification_call_guards`, `private_call_signaling`, `private_active_contact_lookup`. The last migration corrects the profile-RLS lookup uncovered by the rollback test; retain all three when recreating schema. SQL source files reflect that sequence.
- Rollback-only SQL passed for chat replay and private isolation, attachment ownership, preference/read persistence, call transitions, signaling scope and disabled-account denial. No real messages or calls were sent. 23 new local behaviors and all previous suites pass; desktop/mobile local browser fixtures include draft preservation. Team positive-path/live Storage/media/Realtime E2E still needs real accounts and configured official teams.
