...
window.roleAwareBootstrap=async function(){const id=CURRENT_PROFILE?.id,epoch=sessionEpoch;if(!id||CURRENT_AUTH_USER?.id!==id)throw Error('يلزم حساب معتمد لتحميل البيانات');if(actor!==id){clearView();actor=id;}try{await bootstrapBefore.apply(this,arguments);if(epoch!==sessionEpoch||CURRENT_PROFILE?.id!==id||CURRENT_AUTH_USER?.id!==id)throw Error('تغير الحساب أثناء تحميل البيانات');}catch(e){if(epoch===sessionEpoch&&CURRENT_PROFILE?.id===id)mfInvalidateSessionView();throw e;}};
...
window.MF_NEXA_RELEASE='2026-09-17-r10';
})();
